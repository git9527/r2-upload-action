...
### Basic Usage
```yaml
- uses: git9527/r2-upload-action@latest
  with:
...
In this example, `glob-pattern` is stored in the root of the bucket.
...
| `glob-pattern`         | **(Required)** The glob pattern to upload to the Cloudflare R2 bucket.                              | *N/A* |
